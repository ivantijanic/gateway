<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Debit.aspx.cs" Inherits="NETframeworkPaymentGateway.WebForm1" %>
...
<body style="background-color: #63E9FC">
    <%
        ...
        bool isWithRegister = false;
        if (!String.IsNullOrEmpty(Request.Form["withRegister"]))
        {
            isWithRegister = bool.Parse(Request.Form["withRegister"]);
        }


        bool isCardOnFile = false;
        if (!String.IsNullOrEmpty(Request.Form["CardOnFile"]))
        {
            isCardOnFile = bool.Parse(Request.Form["CardOnFile"]);;
        }
        ...
        transactionType myTransaction = new transactionType
        {
            ...
            language = "sl",
            ItemElementName = ItemChoiceType1.debit,
            Item = new debitType
            {
                transactionId = "D-" + Guid.NewGuid().ToString(),
                currency = "EUR",
